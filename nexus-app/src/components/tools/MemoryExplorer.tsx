...
export function MemoryExplorer() {
    ...
    const TIERS: TierInfo[] = useMemo(() => {
        ...
        // Prefer real kernel tier counts (persistent LanceDB rows) over what was
        // fetched into the list; fall back to fetched counts when stats absent.
        const shortTermCount = kernelMemStats?.short_term_count ?? tierCounts[1];
        const longTermCount = kernelMemStats?.long_term_count ?? tierCounts[2];

        return TIER_DEFS.map((def, i) => {
            const count = i === 0 ? workingCount
                : i === 1 ? shortTermCount
                : i === 2 ? longTermCount
                : tierCounts[3];
                        ...
                        <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-5 space-y-5">
                            ...
                            {!loading && memories.length === 0 && selectedTier === null && (
                                <div className="text-center py-8 text-muted-foreground">
                                    ...
                                    {totalEntries > 0 ? (
                                        <>
                                            <p className="text-xs">{totalEntries.toLocaleString()} memories stored.</p>
                                            <p className="text-[10px] opacity-60 mt-1">Use Semantic Search to explore them.</p>
                                        </>
                                    ) : (
                                        <>
