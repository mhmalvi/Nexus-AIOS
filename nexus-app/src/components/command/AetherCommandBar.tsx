...
export function AetherCommandBar() {
  ...
  const predictions = [
    ...
    { label: "Trigger Voice", icon: BoxSelect, type: "system" }
  ];

  const filteredPredictions = input ? predictions.filter(p => p.label.toLowerCase().includes(input.toLowerCase())) : predictions.slice(0, 5);
          ...
          >
            ...
            <div className="w-full relative glass-panel rounded-full p-2.5 flex items-center gap-4 pl-6 shadow-2xl ring-1 ring-white/20 bg-background/80 backdrop-blur-3xl">
              ...
              <div className="shrink-0">
                <AetherPulse state={agent.isThinking ? 'thinking' : input ? 'action' : 'idle'} size={28} />
              </div>
