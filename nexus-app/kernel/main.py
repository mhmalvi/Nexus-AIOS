...
class AetherKernel:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # Shared with the `aether` CLI: read the SAME config key (memory.db_path)
        # the CLI uses so the GUI kernel and the terminal CLI share one store.
        _mem_cfg = self.config.get("memory", {}) or {}
        self.memory = MemoryManager(
            db_path=_mem_cfg.get(
                "db_path",
                self.config.get("lancedb_path", "~/.aether/memory/aether.lance"),
            )
        )
    ...
    async def _handle_security(self, message: KernelMessage) -> KernelResponse:
        ...
        if action == "destruct_status":
            ...
            # Build real findings from (1) the supervisor audit log and
            # (2) config-based security-posture checks.
            findings = []
            try:
                log = self.supervisor.get_audit_log(limit=50) if getattr(self, "supervisor", None) else []
                for i, entry in enumerate(log):
                    e = entry if isinstance(entry, dict) else getattr(entry, "__dict__", None) or {"detail": str(entry)}
                    blocked = bool(e.get("blocked") or e.get("is_failure")) or ("block" in str(e).lower())
                    findings.append({
                        "id": e.get("id", f"audit_{i}"),
                        "severity": "high" if blocked else "info",
                        "category": "Supervisor",
                        "title": str(e.get("reason") or e.get("action") or "Audit entry")[:120],
            ...
            posture = [
                ("Safety checker", self.config.get("enable_safety_checker", True), "Regex/blacklist pre-screen of actions"),
                ("AST command audit", self.config.get("enable_ast_command_audit", True), "Deep structural analysis of shell commands"),
                ("HIL approval (high-risk)", self.config.get("require_approval_for_high_risk", True), "Human approval required for high/critical actions"),
            ]
            for name, enabled, desc in posture:
                findings.append({
                    "id": f"posture_{name.replace(' ', '_').lower()}",
                    "severity": "info" if enabled else "medium",
                    "category": "Posture",
                    "title": f"{name}: {'enabled' if enabled else 'DISABLED'}",
                    "description": desc,
                    "timestamp": "",
