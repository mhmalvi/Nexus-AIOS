...
class ManagerAgent:
    ...
    def __init__(self, brain, worker, security_auditor=None, code_architect=None, researcher=None, qa_engineer=None, failure_limit: int = 3, memory=None, event_emitter=None):
        self.brain = brain
        self.worker = worker
        ...
        # Optional Thought/Action/Observation sink. Signature:
        #   event_emitter(tao_type: str, content: str, step_id: str|None, details: dict)
        # When set (e.g. the CLI's pretty printer), TAO events are routed here
        # instead of being printed to stdout as JSON (the GUI/IPC default).
    ...
    async def execute_task(self, description: str, auto_approve: bool = False) -> Dict[str, Any]:
        ...
        # Guard: a planner that produced no steps must NOT be reported as a
        # vacuous success (all([]) == True). Fail explicitly so the user knows.
        if not getattr(plan, "steps", None):
            self._emit_tao("OBSERVATION", "Planner produced no executable steps")
            result = {
                "success": False,
                "task": description,
                "steps_executed": 0,
                "parallel_waves": 0,
                "results": [],
                "error": "The planner could not produce an executable plan for this task. "
                         "Try rephrasing it as a concrete action.",
            }
            await self._persist_task_result(description, result)
            return result
...
Unified Recommendation:"""
    ...
    def _emit_tao(self, tao_type: str, content: str, step_id: str = None, details: Dict[str, Any] = None):
        """Emit a Thought-Action-Observation event.

        If an event_emitter was injected (e.g. the CLI), route the event there.
        Otherwise fall back to the stdout JSON frame the GUI/IPC consumes.
        """
        if self.event_emitter is not None:
            try:
                self.event_emitter(tao_type, content, step_id, details or {})
                return
            except Exception:
                pass  # fall through to stdout emission
