...
try:
    ...
    # CrewAI is an OPTIONAL orchestration adapter — the native Nexus executor
    # (ManagerAgent -> WorkerAgent -> Toolbox) does not require it. Keep this at
    # debug level so it never spams the CLI/console when crewai isn't installed.
    logger.debug("CrewAI not installed (optional). Install with: pip install crewai")
