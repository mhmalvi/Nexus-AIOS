...
class CloudLLMEngine:
    ...
    def get_active_provider_info(self) -> Dict[str, Any]:
        """Return info about the active provider (for status bar).

        Before any generation has run, last_provider is None — fall back to the
        provider/model that *would* be used next (the first resolved candidate),
        so the status reflects the configured default (e.g. local Ollama) instead
        of "none".
        """
