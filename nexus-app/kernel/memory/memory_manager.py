...
class MemoryManager:
    ...
    def __init__(
        self,
        db_path: str = "~/.aether/memory/aether.lance",
        working_memory_limit: int = 10,
        short_term_ttl_hours: int = 24 * 7,  # 1 week
        embedding_model: str = "nomic-embed-text",
        session_dir: str = "~/.aether/sessions"
    ):
        # Expand ~ so the GUI kernel and the `aether` CLI resolve to the SAME
        # absolute store and share memory (previously they used different paths).
        ...
        # Initialize stores
        self.db_store = LanceDBStore(self.db_path, embedding_model)
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Get memory system statistics (incl. persistent tier row counts)."""
        ...
        # Best-effort row counts for the persistent LanceDB tiers so the UI
        # overview reflects real data (lancedb.connect + count_rows are sync).
        try:
            db = self.db_store.db
            if db is None:
                import lancedb
                db = lancedb.connect(self.db_store.db_path)
            for key, table_name in (("short_term_count", "short_term_memory"),
                                    ("long_term_count", "long_term_memory")):
                try:
                    stats[key] = db.open_table(table_name).count_rows()
                except Exception:
                    stats[key] = 0
        except Exception:
            pass
