...
cat > ${NEXUS_HOME}/bin/aether <<'EOF'
#!/bin/bash
# AETHER agentic CLI — interactive REPL + full agentic executor.
export PYTHONPATH=/opt/nexus/app
export NEXUS_HOME=/opt/nexus
export NEXUS_DATA=/var/lib/nexus
exec /opt/nexus/.venv/bin/python /opt/nexus/app/kernel/aether_cli.py "$@"
EOF
...
ln -sf ${NEXUS_HOME}/bin/aether /usr/local/bin/aether
echo "   ✅ Executables: nexus-core, nexus-kernel, nexus-cli, aether"
